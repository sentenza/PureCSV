...
package object safe {
  ...
  trait CSVReader[A] {
    ...
    def readCSVFromFile(f: File, skipHeader: Boolean = false): List[Try[A]] = {
    ...
    def readCSVFromFileName(fileName: String, skipHeader: Boolean = false): List[Try[A]] = {
